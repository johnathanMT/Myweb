...
// Bake Open Graph / title tags for the /jyotish route so social crawlers
...
// → dist/jyotish.html with the Jyotish meta swapped in. vercel.json rewrites
// `/jyotish` → `/jyotish.html`, so the same JS bundle still loads — browsers get
...
writeFileSync(resolve(dist, 'jyotish.html'), html)
console.log('[prerender-og] wrote dist/jyotish.html with Jyotish OG tags')
